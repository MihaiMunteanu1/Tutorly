...
export function SubjectAvatarsPage() {
      ...
      try {
        const entries = await Promise.all(
          work.map(async (w) => {
            ...
            const name = (preview?.resolvedName ?? w.fallbackName ?? w.avatarId).trim();

            return [w.key, { imageUrl, name }] as const;
          })
        );
  ...
  function onCreateOwnAvatar() {
    navigate("/create-yourself");
  }
            ...
            >
              <div
                style={{
                  ...
                  background:
                    "linear-gradient(180deg, rgba(17,24,39,0.5), rgba(11,18,32,0.9))",
                  display: "flex",
                  alignItems: "center",
...
>
        ...
        >
          ...
          >
            <div style={{ textAlign: "center" }}>
              <div style={{ fontSize: 13, color: "#9ca3af" }}>Create your own avatar</div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            <div style={titleStyle}>Create your own avatar</div>
            <div style={smallMuted}>Generate a photo avatar and select the main image.</div>
            <div style={{ fontSize: 12, color: "#cbd5e1", lineHeight: 1.35 }}>
              Use our photo avatar generation, then proceed to voice selection.
            </div>

            <div style={{ marginTop: 10, fontSize: 12, color: "#d1d5db" }}>Avatar: Photo </div>
            <div style={{ fontSize: 12, color: "#d1d5db" }}>Voice: Pick next</div>
          </div>
        </button>
        <button
          onClick={onCustomise}
        ...
        >
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            ...
            <div style={smallMuted}>Open Avatar Selection Page to pick any avatar/voice.</div>
            <div style={{ fontSize: 12, color: "#cbd5e1", lineHeight: 1.35 }}>
              Select a different character, voice, and style to match your lesson.
            </div>

            <div style={{ marginTop: 10, fontSize: 12, color: "#d1d5db" }}>Avatar: Any</div>
